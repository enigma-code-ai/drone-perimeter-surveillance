...
class DroneController(Node):
    ...
    def _initialize_pid_controllers(self):
        ...
        self.pos_x_pid = PIDController(
            kp=1.5, ki=0.0, kd=0.5, max_output=10.0, min_output=-10.0, max_integral=5.0
        )
        ...
        self.pos_z_pid = PIDController(
            kp=2.0, ki=0.1, kd=1.0, max_output=20.0, min_output=-20.0, max_integral=10.0
        )

        # Attitude Controllers (Angle → Rate)
        self.roll_pid = PIDController(
            kp=7.0, ki=0.0, kd=0.0, max_output=50.0, min_output=-50.0
        )
        ...
        self.yaw_pid = PIDController(
            kp=5.0, ki=0.0, kd=0.0, max_output=30.0, min_output=-30.0
        )
        ...
        self.yaw_rate_pid = PIDController(
            kp=0.2, ki=0.05, kd=0.0, max_output=1.0, min_output=-1.0, max_integral=0.5
        )
    ...
    def _setup_subscribers(self):
        ...
        self.imu_sub = self.create_subscription(
            Imu, "/drone/imu/data", self.imu_callback, qos
        )

        self.pose_sub = self.create_subscription(
            PoseStamped, "/drone/pose", self.pose_callback, qos
        )
    ...
    def control_loop(self):
        ...
        acc_z = (
            self.pos_z_pid.compute(position_error[2], self.control_dt) + self.gravity
        )
