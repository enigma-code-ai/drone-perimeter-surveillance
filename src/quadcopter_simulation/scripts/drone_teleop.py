...
class AdvancedDroneTeleop(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            f"Flight modes available: {[mode.value for mode in FlightMode]}"
        )
    ...
    def setup_terminal(self):
        ...
        self.get_logger().info(
            f"Terminal setup - stdin.isatty(): {sys.stdin.isatty()}, HAS_TERMIOS: {HAS_TERMIOS}"
        )
    ...
    def setup_rich_ui(self):
        ...
        self.layout["left"].split_column(Layout(name="status"), Layout(name="controls"))

        self.layout["center"].split_column(
            Layout(name="telemetry"), Layout(name="mission")
        )

        self.layout["right"].split_column(
            Layout(name="safety"), Layout(name="performance")
        )
    ...
    def keyboard_loop(self):
        ...
        if not self.interactive_mode:
            self.get_logger().warn(
                "Interactive mode disabled - keyboard input not available"
            )
            return
    ...
    def process_keyboard_input(self, key: str):
        ...
        if self.drone_state.armed and self.drone_state.flight_mode in [
            FlightMode.MANUAL,
            ...
            if key == "w":
                ...
                self.current_twist.linear.x = (
                    self.settings.max_linear_speed * self.settings.linear_sensitivity
                )
                ...
                self.current_twist.angular.z = (
                    self.settings.max_angular_speed * self.settings.angular_sensitivity
                )
                self.get_logger().info(
                    f"Publishing: angular.z={self.current_twist.angular.z}"
                )
                return
            elif key == "e":
                ...
                self.current_twist.linear.z = (
                    self.settings.max_vertical_speed
                    * self.settings.vertical_sensitivity
                )
                self.current_twist.angular.z = 0.0
        ...
        elif key_lower == "s" and not (
            self.drone_state.armed
            and self.drone_state.flight_mode
            in [FlightMode.MANUAL, FlightMode.STABILIZED]
        ):
            self.set_flight_mode(FlightMode.STABILIZED)
        elif key_lower == "a" and not (
            self.drone_state.armed
            and self.drone_state.flight_mode
            in [FlightMode.MANUAL, FlightMode.STABILIZED]
        ):
    ...
    def control_loop(self):
        ...
        if self.drone_state.flight_mode == FlightMode.MANUAL:
            ...
            if (
                abs(self.current_twist.linear.x) > 0.1
                or abs(self.current_twist.linear.y) > 0.1
                or abs(self.current_twist.linear.z) > 0.1
                or abs(self.current_twist.angular.z) > 0.1
            ):
                self.get_logger().info(
                    f"Publishing cmd_vel: x={self.current_twist.linear.x:.2f}, y={self.current_twist.linear.y:.2f}, z={self.current_twist.linear.z:.2f}, yaw={self.current_twist.angular.z:.2f}"
                )
    ...
    def safety_check(self):
        ...
        if self.settings.geofence_enabled and self.drone_state.position:
            ...
            if (
                pos.x < self.settings.geofence_min_x
                or pos.x > self.settings.geofence_max_x
                or pos.y < self.settings.geofence_min_y
                or pos.y > self.settings.geofence_max_y
            ):
    ...
    def pose_callback(self, msg: PoseStamped):
        ...
        if self.last_position.x != 0 or self.last_position.y != 0:
            distance = self.calculate_distance(
                self.drone_state.position, self.last_position
            )
    ...
    def altitude_hold_control(self):
        ...
        altitude_error = (
            self.target_position.pose.position.z - self.drone_state.position.z
        )
        twist.linear.z = max(
            -1.0, min(1.0, altitude_error * 0.5)
        )  # Simple P controller
    ...
    def mission_control(self):
        ...
        if not self.mission_waypoints or self.current_waypoint_index >= len(
            self.mission_waypoints
        ):
    ...
    def toggle_arm(self):
        ...
        if self.drone_state.armed:
            ...
            if (
                self.drone_state.battery_percentage > 0
                and self.drone_state.battery_percentage < 10
            ):
    ...
    def add_waypoint(self):
        ...
        self.get_logger().info(
            f"Waypoint {len(self.mission_waypoints)} added at "
    ...
    def begin_mission(self):
        ...
        self.set_flight_mode(FlightMode.AUTO_MISSION)
        self.get_logger().info(
            f"Mission started with {len(self.mission_waypoints)} waypoints"
        )
    ...
    def adjust_speed(self, delta: float):
        ...
        self.settings.linear_sensitivity = max(
            0.1, min(2.0, self.settings.linear_sensitivity + delta)
        )
        self.settings.angular_sensitivity = max(
            0.1, min(2.0, self.settings.angular_sensitivity + delta)
        )

        self.get_logger().info(
            f"Speed adjusted: linear={self.settings.linear_sensitivity:.1f}, "
...
==================================
    ...
    def shutdown(self):
        ...
        if self.interactive_mode and HAS_TERMIOS:
            try:
                ...
                pass  # Terminal restoration failed, but continue gracefully
    ...
    def save_flight_log(self):
        ...
        # Try to use a project-specific log directory first
        try:
    ...
    def update_rich_display(self):
        ...
        try:
            ...
            header_text.append(
                f" | Mode: {self.drone_state.flight_mode.value}", style="bold green"
            )
            header_text.append(
                f" | Status: {self.drone_state.safety_status.value}",
                style=(
                    "bold red"
                    if self.drone_state.safety_status != SafetyStatus.SAFE
                    else "bold green"
                ),
            )
            ...
            status_table.add_row(
                "Distance", f"{self.drone_state.distance_traveled:.1f}m"
            )
...
[green]T[/green] Takeoff  [green]L[/green] Land  [green]Space[/green] Arm  [green]ESC[/green] Emergency
            ...
            if self.mission_waypoints and self.current_waypoint_index < len(
                self.mission_waypoints
            ):
            ...
            home_distance = self.calculate_distance(
                pos, self.home_position.pose.position
            )
            ...
            safety_table = Table(
                title=f"Safety ({self.drone_state.safety_status.value})",
                title_style=safety_color,
                box=box.SIMPLE,
            )
            ...
            safety_table.add_row(
                "Battery",
                f"[{battery_color}]{self.drone_state.battery_percentage:.1f}%[/{battery_color}]",
            )
            ...
            if self.drone_state.geofence_violations:
                violations_text = (
                    "[red]" + ", ".join(self.drone_state.geofence_violations) + "[/red]"
                )
            else:
            ...
            performance_table.add_row(
                "Speed", f"{math.sqrt(vel[0]**2 + vel[1]**2):.2f}m/s"
            )
            performance_table.add_row(
                "Max Alt", f"{self.drone_state.max_altitude:.1f}m"
            )
            performance_table.add_row(
                "Total Dist", f"{self.drone_state.distance_traveled:.1f}m"
            )
            performance_table.add_row("Log Entries", str(len(self.flight_log)))

            # Control sensitivity
            performance_table.add_row(
                "Linear Sens", f"{self.settings.linear_sensitivity:.1f}"
            )
            performance_table.add_row(
                "Angular Sens", f"{self.settings.angular_sensitivity:.1f}"
            )
            ...
            footer_text.append(
                "[M]ode [T]akeoff [L]and [Space]Arm [N]avWP [B]eginMission [H]ome [I]nfo [ESC]Emergency [Ctrl+C]Quit",
                style="dim",
            )
